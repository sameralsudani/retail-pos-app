...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // Get key metrics
      const [salesStats, productStats, customerStats, userStats] =
        await Promise.all([
          // Sales statistics
          (async () => {
            // Aggregate for all time/date range
            const mainStats = await Transaction.aggregate([
              { $match: dateQuery },
              {
                $group: {
                  _id: null,
            ...
            const todayStats = await Transaction.aggregate([
              { $match: { createdAt: { $gte: today, $lt: tomorrow } } },
              {
                $group: {
                  _id: null,
                  totalItemsSoldToday: { $sum: { $sum: "$items.quantity" } },
                },
              },
            ]);
            ...
            return {
              ...mainStats[0],
              totalItemsSoldToday: todayStats[0]?.totalItemsSoldToday || 0,
            };
          })(),
      ...
      res.json({
        ...
        data: {
          ...
          customers,
          users: {
            totalUsers: Object.values(roleStats).reduce(
              (sum, count) => sum + count,
              0
            ),
...
router.get(
  ...
  async (req, res) => {
    try {
      const [inventoryStats, lowStockProducts, categoryBreakdown] =
        await Promise.all([
  ...
  async (req, res) => {
    try {
      ...
      const [customerStats, topCustomers, loyaltyStats] = await Promise.all([
        ...
        Customer.find({ isActive: true }).sort({ totalSpent: -1 }).limit(limit),
