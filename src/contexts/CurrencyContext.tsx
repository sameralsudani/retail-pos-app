...
export const CurrencyProvider: React.FC<CurrencyProviderProps> = ({ children }) => {
  ...
  const getInitialCurrency = () => {
    const savedCode = typeof window !== 'undefined' ? localStorage.getItem('currencyCode') : null;
    const found = CURRENCIES.find(c => c.code === savedCode);
    return found || CURRENCIES[0];
  };
  const [displayCurrency, setDisplayCurrencyState] = useState<Currency>(getInitialCurrency());
  ...
  const convertAmount = (amount: number, fromCurrency: Currency, toCurrency: Currency): number => {
    ...
    const fromRate = fromCurrency.code === 'IQD' ? exchangeRate : fromCurrency.rate;
    ...
    const convertedAmount = toCurrency.code === 'USD' ? usdAmount : usdAmount * toRate;
