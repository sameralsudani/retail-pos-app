...
const NewSaleModal: React.FC<InvoiceModalProps> = ({
  selectedCustomer,
  setSelectedCustomer,
  closeInvoiceModal,
  invoiceStep,
  ...
  const [selected, setSelected] =
    useState<typeof selectedCustomer>(selectedCustomer);
  ...
  // If no customer selected, show customer picker first
  if (!selected) {
    return (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg w-full max-w-md mx-4 max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="p-4">
            <input
              ...
              className="w-full mb-4 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
            />
            <div className="space-y-2 max-h-60 overflow-y-auto">
              <button
                className="w-full text-left px-4 py-2 rounded hover:bg-blue-50 dark:hover:bg-blue-900/20 border border-gray-200 dark:border-gray-700 mb-2"
                onClick={() => {
                  setSelected({
                    name:
                      t("dashboard.walkin.customer") ||
                      t("customer.walk.in") ||
                      "Walk-in Customer",
                  });
                  setSelectedCustomer({
              ...
              {customerList
                ...
                .map((c) => (
                  <button
                    key={c._id}
                    className="w-full text-left px-4 py-2 rounded hover:bg-blue-50 dark:hover:bg-blue-900/20 border border-gray-200 dark:border-gray-700"
                    onClick={() => {
                      setSelected(c);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg w-full max-w-4xl mx-4 max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">
              {t("customers.create.invoice")} - {selected?.name}
            </h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {t("customers.invoice.step")}{" "}
              {invoiceStep === "products"
                ? "1"
                : invoiceStep === "review"
                ? "2"
                : "3"}{" "}
              {t("customers.invoice.of")} 3
            </p>
          </div>
          <button
            onClick={closeInvoiceModal}
                ...
                <div className="flex space-x-3">
                  ...
                  {parseFloat(amountPaid) < total && (
                    <div className="mt-2 text-sm text-red-600 dark:text-red-400 text-center">
                      {t("customers.invoice.due.amount")}: $
                      {(total - (parseFloat(amountPaid) || 0)).toFixed(2)}
                    </div>
                  )}
                </div>
              </div>
