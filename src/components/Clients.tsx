    ...
    try {
      ...
      if (response.success) {
        const mappedClients = response.data.map(apiClient => ({
          ...
          address: `${apiClient.address?.street || ''} ${apiClient.address?.city || ''} ${apiClient.address?.state || ''} ${apiClient.address?.zipCode || ''}`.trim() || 'No address provided',
          totalRevenue: apiClient.totalRevenue || 0,
          activeInvoices: apiClient.activeInvoices || 0,
          lastTransaction: apiClient.lastTransaction ? new Date(apiClient.lastTransaction).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
          status: apiClient.status || 'active',
          projects: apiClient.projects || 0,
          avatar: apiClient.avatar || apiClient.name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2),
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-6 space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-orange-600">{stats.totalActiveInvoices}</p>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">+{stats.newClientsThisMonth}</p>
        ...
        {filteredClients.length === 0 && !isLoading && (
          <div className="text-center py-12">
            ...
            <h3 className="mt-2 text-sm font-medium text-gray-900">{t('clients.empty.title')}</h3>
            <p className="mt-1 text-sm text-gray-500">{t('clients.empty.subtitle')}</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <form className="space-y-4">
              ...
              <div className={`flex pt-4 ${document.documentElement.dir === 'rtl' ? 'space-x-reverse space-x-3' : 'space-x-3'}`}>
                ...
                >
                  {isSubmitting ? t('clients.form.adding') : t('clients.form.add')}
                </button>
              </div>
            </form>
          </div>
      ...
      {showEditModal && selectedClient && canEdit && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">{t('clients.edit.title')}</h2>
            <form className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('clients.form.company.name')}</label>
                <input
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('clients.form.status')}</label>
                <select
              ...
              <div className={`flex pt-4 ${document.documentElement.dir === 'rtl' ? 'space-x-reverse space-x-3' : 'space-x-3'}`}>
                ...
                >
                  {isSubmitting ? t('clients.form.saving') : t('clients.form.save')}
                </button>
              </div>
            </form>
          </div>
